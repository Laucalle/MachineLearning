...
Se han marcado en rojo los modelos con los que se obtiene el mínimo error, en verde los obtenidos con el estimador $C_p$ proporcionado por \texttt{regsubsets}. Este estimador, además de utilizar el error cuadrático estimado del ajuste realizado por un subconjunto de predictores, añade una penalización al número de características escogidas. Su objetivo es evitar el sobreajuste, puesto que un número de predictores elevado puede resultar en que características que no son de relevancia jueguen un papel importante en el modelo, obteniendo resultados pobres al aplicarse a nuevas observaciones. _BIC_ es un estimador similar, que se diferencia del primero en el cálculo de la penalización. Para ello, tiene en cuenta el número total de observaciones además del número de características seleccionadas, por lo que generalmente elige modelos con menos predictores que $C_p$. 

En la siguiente tabla se muestran, para los distintos tipos de ajuste y datos, que modelos se han seleccionado siguiendo los tres criterios descritos. Para cada modelo se proporcionan el $E_{test}$ y el número de predictores utilizado.
...
| Ajustes lm sin PCA  | 6.705729       | 28     | 6.901042          | 34     | 6.966146           | 26     |
| Ajustes glm sin PCA | 6.25           | 31     | 6.315104          | 34     | 6.510417           | 26     |

A la vista de estos resultados parece razonable elegir los modelos que dan el menor error en la partición de test. Sin embargo en algunos casos como en el ajuste de regresión lineal con PCA, en el que la diferencia entre el error mínimo y el sugerido por el estimador $C_p$ no es muy significativa, podemos preferir este segundo modelo ya que su dimensionalidad es menor. 